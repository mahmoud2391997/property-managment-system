...
export function PendingAssignmentBanner ({
  ...
  const isAccepting = actionLoading === 'accepting'
...
export function AddCommentSection ({
  ...
  // Derive loading state from actionLoading prop
  const loading = actionLoading === 'commenting'
...
type ResolveTaskDialogProps = {
  onSubmit: (report: string, attachmentFile?: File) => Promise<void>
  loading?: boolean
  disabled?: boolean
}
...
export function ResolveTaskDialog ({
  ...
  const handleSubmit = async () => {
    ...
    if (report.trim().length < MIN_REPORT_LENGTH) {
      FeedbackToasts.warning('Validation', `Report must be at least ${MIN_REPORT_LENGTH} characters`)
      return
    }

    try {
      await onSubmit(report.trim(), attachmentFile || undefined)
      ...
      // Error is handled by parent, don't close dialog
    }
  }
      ...
      <div className='space-y-4'>
        <InputGroup label='Resolution Report' isRequired>
          <Textarea
            ...
            placeholder='Describe what was done to complete this task (min 10 characters)...'
