...
export default function TaskStaffAssignedSection({
  assignedStaff,
  pendingStaff,
  staffList,
  currentStaffId,
  ...
  const [unassigningStaff, setUnassigningStaff] = useState<StaffMember | null>(null)

  // Derive loading states from actionLoading prop
  const isAssigning = actionLoading === 'assigning'
  const isSendingRequest = actionLoading === 'sending_request'
  const isUnassigning = actionLoading === 'unassigning'
  const isCancelling = actionLoading === 'cancelling'
  const loading = isAssigning || isSendingRequest || isUnassigning || isCancelling
  ...
  const handleSendRequest = () => {
    ...
    // Don't clear state here - it will be reset when props change
  }

  const handleConfirmUnassign = () => {
    if (!unassignReason.trim()) return
    // Store the staff being unassigned so we can show their name during loading
    if (assignedStaff) {
      setUnassigningStaff(assignedStaff)
    }
  ...
  // Reset forms when assignment state changes (after successful API call)
  useEffect(() => {
    // Only reset if not in a loading state
    if (!isUnassigning && !isCancelling && !isAssigning && !isSendingRequest) {
  ...
  }, [assignedStaff, pendingStaff, isUnassigning, isCancelling, isAssigning, isSendingRequest])

  // Assigned State (also show during unassigning to maintain loading UI)
  if ((assignmentState === 'assigned' && assignedStaff) || isUnassigning) {
    const staffToShow = assignedStaff || unassigningStaff || { id: '', name: 'Staff', avatar: undefined }
    return (
        ...
        {!showUnassignConfirmation ? (
          <div className='flex items-center justify-between p-2 rounded-lg bg-(--background-secondary) border border-(--border-default)'>
            <div className='flex items-center gap-3'>
              <UserAvatar name={staffToShow.name} imgSrc={staffToShow.avatar} size={32} />
          ...
          <div className='p-3 rounded-lg bg-red-50 border border-red-200'>
            <p className='texts-body-small text-(--text-primary) mb-3'>
              Unassign{' '}
              <span className='texts-body-small-medium'>{staffToShow.name}</span>{' '}
              from this task?
            </p>
            <textarea
              value={unassignReason}
              onChange={e => setUnassignReason(e.target.value)}
              placeholder='Reason for unassigning (min 10 characters)...'
            ...
            <div className='flex items-center gap-2'>
              <button
                ...
                disabled={loading || unassignReason.trim().length < 10}
        ...
        {!showCancelConfirmation ? (
          ...
          <div className='p-3 rounded-lg bg-red-50 border border-red-200'>
            ...
            <textarea
              ...
              placeholder='Reason for cancelling (min 10 characters)...'
              className='w-full p-2 mb-3 texts-body-small bg-white border border-red-200 rounded-md resize-none min-h-[60px] focus:outline-none focus:ring-1 focus:ring-red-300'
            />
            <div className='flex items-center gap-2'>
              <button
        ...
        <div className='p-3 rounded-lg bg-blue-50 border border-blue-200'>
          ...
          <div className='flex items-center gap-2'>
            ...
            >
              ...
              {selectedStaffId === currentStaffId
                ? (isAssigning ? 'Assigning...' : 'Assign Myself')
