...
const EditRoomLease = () => {
  const { id: roomId, leaseId } = useParams<{ id: string; leaseId: string }>()

  const [roomInfo, setRoomInfo] = useState<{ roomTitle: string; propertyCode: string } | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = `/rooms/${roomId}/leases/${leaseId}/details`
      }, 1500)
    } catch (error: any) {
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
        <AddPageHead
          crumb_items={[
            ...
            { label: roomInfo ? `${roomInfo.propertyCode}(${roomInfo.roomTitle})` : '', href: `/rooms/${roomId}/leases` },
            { label: 'Edit Lease' }
          ]}
          isCrumbLoading={false}
          title='Edit Lease'
          subtitle={roomInfo ? `Update lease for ${roomInfo.roomTitle}` : 'Update lease details and terms'}
          className='mb-7.5'
