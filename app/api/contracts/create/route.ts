    ...
    const result = await prisma.$transaction(async tx => {
      ...
      if (firstMonthRentalCharge) {
        ...
        if (is_paid) {
          ...
          // ============================================
          // STEP 5: Create Next Month Rental Expense (if First Month is Paid)
          // ============================================

          const nextMonthDueDate = new Date(paymentDateTime)
          ...
          const nextMonthRefId = generateExpenseReferenceId(expenseNextSequence)

          const nextMonthExpense = await tx.expenses.create({
            data: {
              reference_id: nextMonthRefId,
              category: 'Contract_Related',
              status: 'Pending',
              due_payment_date: nextMonthDueDate,
              organization_id: staff.organization_id,
              created_by: staff.id,
              charges: {
                create: {
