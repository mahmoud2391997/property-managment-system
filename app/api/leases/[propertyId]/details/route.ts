...
export async function GET(
  ...
  try {
    ...
    const endDate = lease.number_of_months
      ? new Date(new Date(lease.start_date).setMonth(new Date(lease.start_date).getMonth() + lease.number_of_months))
      : null

    // Get upcoming scheduled change (if any)
    const upcomingChange = lease.scheduled_rental_changes.find(sc => sc.status === 'Scheduled')
    ...
    const formattedPayments = payments.map(p => {
      ...
      const paidAmount = p.payment_history.reduce((sum, h) => sum + Number(h.amount), 0)
    ...
    const pendingRental = formattedPayments.find(p => p.type === 'Rental' && p.status === 'Pending')
    const recentPayments = formattedPayments.slice(0, 10) // Last 10 payments
    ...
    const recurringPayments = lease.recurring_configs.map(rc => {
      ...
      return {
        ...
        latest_payment: latestPayment ? {
          ...
          amount: latestPayment.charges.reduce((sum, c) => sum + Number(c.amount), 0)
        } : null
      }
    })
      ...
      lease: {
        ...
        transfer: {
          ...
          transferred_from_lease: lease.leases_leases_transferred_fromToleases ? {
            ...
            reference_id: lease.leases_leases_transferred_fromToleases.reference_id,
          ...
          transferred_to_lease: lease.leases_leases_transferred_toToleases ? {
            ...
            reference_id: lease.leases_leases_transferred_toToleases.reference_id,
