...
export async function POST(
  ...
  try {
    ...
    if (!hasPermission(permissions, 'rooms.initiate_preparation_flow'))
    ...
    // Verify room exists and belongs to the organization
    ...
    // Check room status is Ready
    if (room.status !== 'Ready') {
      return NextResponse.json(
        { error: 'Room must be in Ready status to initiate preparation flow' },
        { status: 400 }
      )
    }

    // Check property status is Ready
    if (room.properties.status !== 'Ready') {
      return NextResponse.json(
        { error: 'Cannot initiate preparation flow - parent property is not ready' },
        { status: 400 }
      )
    }

    // Check no active property lease
    const hasActivePropertyLease = room.properties.leases.some((lease: { start_date: Date; number_of_months: number | null }) => {
    ...
    if (hasActivePropertyLease) {
      return NextResponse.json(
        { error: 'Cannot initiate preparation flow - parent property has an active lease' },
        { status: 400 }
      )
    }

    // Check no active room lease
    const hasActiveRoomLease = room.leases.some((lease: { start_date: Date; number_of_months: number | null }) => {
      const leaseEndDate = lease.number_of_months
        ? new Date(lease.start_date.getTime() + lease.number_of_months * 30 * 24 * 60 * 60 * 1000)
        : null
      const now = new Date()
    ...
    if (hasActiveRoomLease) {
      return NextResponse.json(
        { error: 'Cannot initiate preparation flow - room has an active lease' },
        { status: 400 }
      )
    }

    // Check if there's an existing active flow for this room
    ...
    if (existingFlow) {
      return NextResponse.json(
        { error: 'A preparation flow is already in progress for this room' },
        { status: 400 }
      )
    }
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      // 3. Update room status to Under_Preparation
    ...
    console.error('Error initiating room preparation flow:', error)
    return NextResponse.json(
      { error: 'Failed to initiate room preparation flow' },
      { status: 500 }
    )
  }
}
