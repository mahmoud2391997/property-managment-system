...
export async function POST(
  ...
  try {
    ...
    if (currentType !== 'Preparation') {
      return NextResponse.json({ error: 'This is not a preparation task' }, { status: 400 })
    }
        ...
        if (hasRefundableDeposit) {
          const refundRequestTask = await tx.tasks.create({
            data: {
              ...
              description: 'Review inspection and preparation reports to determine security deposit refund amount.',
              organization_id: organizationId,
    ...
    return NextResponse.json({ error: 'Failed to complete preparation' }, { status: 500 })
  }
}
