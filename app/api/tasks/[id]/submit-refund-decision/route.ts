...
export async function POST (
  ...
  try {
    ...
    if (!hasPermission(permissions, 'tasks.submit_refund_decision'))
    ...
    if (!report || report.trim().length < 10) {
      return NextResponse.json(
        { error: 'Report is required (min 10 characters)' },
        { status: 400 }
      )
    }
    ...
    if (decision === 'partial') {
      if (!charges || !Array.isArray(charges) || charges.length === 0) {
        return NextResponse.json(
          { error: 'Charges are required for partial refund' },
          { status: 400 }
        )
      }
      for (const charge of charges) {
        if (!charge.description || charge.description.trim().length < 3) {
          return NextResponse.json(
            { error: 'Each charge must have a description (min 3 chars)' },
            { status: 400 }
          )
        }
        if (typeof charge.amount !== 'number' || charge.amount <= 0) {
          return NextResponse.json(
            { error: 'Each charge must have a valid amount' },
            { status: 400 }
          )
        }
      }
    ...
    if (currentType !== 'Refund_Request') {
      return NextResponse.json(
        { error: 'This is not a refund request task' },
        { status: 400 }
      )
    }
    ...
    if (
      ...
      currentStatus !== 'Needs_Modification'
    ) {
      return NextResponse.json(
        { error: 'Task must be In Progress or Needs Modification to submit' },
        { status: 400 }
      )
    }
    ...
    if (!assignment || assignment.assigned_id !== staff.id) {
      return NextResponse.json(
        { error: 'Only the assigned staff can submit refund decision' },
        { status: 400 }
      )
    }
    ...
    const depositAmount = refundableCharges.reduce(
      (sum, charge) => sum + charge.amount.toNumber(),
      0
    )
    ...
    const priority: 'Low' | 'Medium' | 'High' | 'Urgent' =
      (task.task_priorities[0]?.priority as
        | 'Low'
        | 'Medium'
        | 'High'
        | 'Urgent') || 'Medium'
    ...
    const result = await prisma.$transaction(
      async tx => {
        ...
        // 4. Handle decision-specific logic
        if (refundDecision.decision === 'forfeit') {
          // For forfeit, complete the flow immediately without review
          // No refund finalization task needed, no expense created (refund amount is 0)

          // Update refund decision status to approved (no review needed for forfeit)
          await tx.refund_decisions.update({
            where: { id: refundDecision.id },
            data: {
              status: 'approved',
          ...
          return { refundFinalizationTaskId: null, flowCompleted: true }
        } else {
          // For full/partial refund, create or reactivate Refund Finalization task
          if (flowInstance.refund_finalization_task_id) {
            ...
            const refundFinalizationTask = await tx.tasks.create({
              data: {
                ...
                description:
                  'Review the refund decision and approve or request modifications.',
                organization_id: organizationId,
            ...
            await tx.task_types.create({
              data: {
                task_id: refundFinalizationTask.id,
          ...
          return { refundFinalizationTaskId, flowCompleted: false }
        }
      },
      {
    ...
    return NextResponse.json(
      { error: 'Failed to submit refund decision' },
      { status: 500 }
    )
  }
}
